...
//		cbcourse.setBounds(225,400,150,30);
...
//			String dep =(String) cbbranch.getSelectedItem();
//			try {
//				String query="insert into teacher values('"+name+"','"+fname+"','"+empid+"','"+dob+"','"+address+"','"+phone+"','"+email+"','"+x+"','"+xi+"','"+adhar+"','"+education+"','"+dep+"')";
//			Conn con=new Conn();
...
public class AddTeacher extends JFrame implements ActionListener {
    ...
    AddTeacher() {
        ...
        labelempid = new JLabel("101" + first4);
        labelempid.setBounds(180, 130, 150, 30);
        labelempid.setFont(new Font("Segoe UI", Font.BOLD, 16));
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == submit) {
            ...
            try {
                String query = "INSERT INTO teacher VALUES('" + name + "','" + fname + "','" + empid + 
                               "','" + dob + "','" + address + "','" + phone + "','" + email + 
                               "','" + x + "','" + xi + "','" + adhar + "','" + education + "','" + dep + "')";
                Conn con = new Conn();
                con.s.executeUpdate(query);
                JOptionPane.showMessageDialog(null, "Teacher Details Inserted Successfully");
                setVisible(false);
